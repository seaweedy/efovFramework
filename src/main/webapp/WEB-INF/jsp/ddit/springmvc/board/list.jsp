...
<head>
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
        <!--
        /* 글 수정 화면 function */
        function fn_egov_select(boardSn) {
        	document.listForm.boardSn.value = boardSn;
           	document.listForm.action = "<c:url value='/board/updateView.do'/>";
           	document.listForm.submit();
        }
        
        /* 글 등록 화면 function */
        function fn_egov_addView() {
           	document.listForm.action = "<c:url value='/board/createView.do'/>";
           	document.listForm.submit();
        }
...
<body style="text-align:center; margin:0 auto; display:inline; padding-top:100px;">
    <form:form commandName="boardVO" id="listForm" name="listForm" method="post">
        <input type="hidden" name="boardSn" />
        <div id="content_pop">
        	<!-- 타이틀 -->
        	<div id="title">
        		<ul>
        		...
        		<table width="100%" border="0" cellpadding="0" cellspacing="0" summary="카테고리ID, 케테고리명, 사용여부, Description, 등록자 표시하는 테이블">
        			...
        			<c:forEach var="result" items="${resultList}" varStatus="status">
            			<tr>
            				...
            				<td align="left" class="listtd"><a href="javascript:fn_egov_select('<c:out value="${result.boardSn}"/>')"><c:out value="${result.title}"/></a></td>
            				<td align="center" class="listtd"><c:out value="${result.writer}"/>&nbsp;</td>
            				<td align="center" class="listtd"><c:out value="${result.openYn}"/>&nbsp;</td>
            			</tr>
        			</c:forEach>
        		</table>
        	</div>
